...
class LaLabsu2netpOutput : MLFeatureProvider {
...
class LaLabsu2netp {
    ...
    class var urlOfModelInThisBundle : URL {
        return Bundle.module.url(forResource: "LaLabsu2netp", withExtension:"mlmodelc")!
    }

    /**
        Construct LaLabsu2netp instance with an existing MLModel object.

        Usually the application does not use this initializer unless it makes a subclass of LaLabsu2netp.
        Such application may want to use `MLModel(contentsOfURL:configuration:)` and `LaLabsu2netp.urlOfModelInThisBundle` to create a MLModel object to pass-in.
    ...
    */
    ...
    /**
        Construct LaLabsu2netp instance with explicit path to mlmodelc file
        - parameters:
    ...
    */
    ...
    /**
        Construct LaLabsu2netp instance asynchronously with optional configuration.
    ...
    */
    class func load(configuration: MLModelConfiguration = MLModelConfiguration(), completionHandler handler: @escaping (Swift.Result<LaLabsu2netp, Error>) -> Void) {
    ...
    */
    ...
    class func load(configuration: MLModelConfiguration = MLModelConfiguration()) async throws -> LaLabsu2netp {
        return try await self.load(contentsOf: self.urlOfModelInThisBundle, configuration: configuration)
    }

    /**
        Construct LaLabsu2netp instance asynchronously with URL of the .mlmodelc directory with optional configuration.
        ...
        - parameters:
          ...
          - handler: the completion handler to be called when the model loading completes successfully or unsuccessfully
    */
    class func load(contentsOf modelURL: URL, configuration: MLModelConfiguration = MLModelConfiguration(), completionHandler handler: @escaping (Swift.Result<LaLabsu2netp, Error>) -> Void) {
        MLModel.load(contentsOf: modelURL, configuration: configuration) { result in
            switch result {
            case .failure(let error):
                handler(.failure(error))
            case .success(let model):
                handler(.success(LaLabsu2netp(model: model)))
            }
        }
    }
    ...
    */
    @available(macOS 12.0, iOS 15.0, tvOS 15.0, watchOS 8.0, *)
    class func load(contentsOf modelURL: URL, configuration: MLModelConfiguration = MLModelConfiguration()) async throws -> LaLabsu2netp {
        let model = try await MLModel.load(contentsOf: modelURL, configuration: configuration)
        return LaLabsu2netp(model: model)
    }
    ...
    /**
        ...
        - parameters:
           - input: the input to the prediction as LaLabsu2netpInput

        - throws: an NSError object that describes the problem

        - returns: the result of the prediction as LaLabsu2netpOutput
    */
    func prediction(input: LaLabsu2netpInput) throws -> LaLabsu2netpOutput {
    ...
    /**
        ...
        - returns: the result of the prediction as LaLabsu2netpOutput
    */
    func prediction(input: LaLabsu2netpInput, options: MLPredictionOptions) throws -> LaLabsu2netpOutput {
        let outFeatures = try model.prediction(from: input, options:options)
        return LaLabsu2netpOutput(features: outFeatures)
    }
    ...
    /**
        ...
        - returns: the result of the prediction as LaLabsu2netpOutput
    */
    func prediction(in_0: CVPixelBuffer) throws -> LaLabsu2netpOutput {
        let input_ = LaLabsu2netpInput(in_0: in_0)
    ...
    /**
        ...
        - parameters:
           - inputs: the inputs to the prediction as [LaLabsu2netpInput]
           - options: prediction options 
        ...
        - returns: the result of the prediction as [LaLabsu2netpOutput]
    */
    func predictions(inputs: [LaLabsu2netpInput], options: MLPredictionOptions = MLPredictionOptions()) throws -> [LaLabsu2netpOutput] {
        let batchIn = MLArrayBatchProvider(array: inputs)
        let batchOut = try model.predictions(from: batchIn, options: options)
        var results : [LaLabsu2netpOutput] = []
        results.reserveCapacity(inputs.count)
        for i in 0..<batchOut.count {
            let outProvider = batchOut.features(at: i)
            let result =  LaLabsu2netpOutput(features: outProvider)
